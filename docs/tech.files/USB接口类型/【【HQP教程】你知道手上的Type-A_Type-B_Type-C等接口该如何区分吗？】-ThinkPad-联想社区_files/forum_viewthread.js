...
function fastpmpostappendreply(pmtype,reppid) {
	if($('fastpostrefresh') != null) {
		setcookie('fastpostrefresh', $('fastpostrefresh').checked ? 1 : 0, 2592000);
		if($('fastpostrefresh').checked) {
			location.href = 'forum.php?mod=redirect&tid='+tid+'&goto=lastpost&random=' + Math.random() + '#lastpost';
	...
	div.className = '';
	if(pmtype == 'live'){
		$('list_replyid_'+reppid).appendChild(div);
	}
	...
	*/
	hideWindow('pmpostreply');
	showCreditPrompt();
}

function cs() {
	var objv = $('review_comment_91');
	objv.innerHTML = parseInt(objv.innerHTML ? objv.innerHTML : 0) + 1;
	$('review_comment_91').innerHTML = parseInt($('review_comment_91')) + 1;
	return false;
}

function fastthreadappendreply() {
	if($('fastpostrefresh') != null) {
		setcookie('fastpostrefresh', $('fastpostrefresh').checked ? 1 : 0, 2592000);
		if($('fastpostrefresh').checked) {
			location.href = 'forum.php?mod=redirect&tid='+tid+'&goto=lastpost&random=' + Math.random() + '#lastpost';
	...
	newpos = fetchOffset($('thread_new'));
	document.documentElement.scrollTop = newpos['top'];
	$('thread_new').style.display = '';
	$('thread_new').id = '';
	div = document.createElement('div');
	div.id = 'thread_new';
	div.style.display = 'none';
	div.className = '';
	jq("#tlistreply").prepend(div);
	$('fastpmthreadsubmit').disabled = false;
	if($('fastthreadmessage')) {
		$('fastthreadmessage').value = '';
	} else {
		editdoc.body.innerHTML = BROWSER.firefox ? '<br />' : '';
	}
	if(jq('input[name=subject]')) {
		jq('input[name=subject]').val("请在此输入");
		jq('input[name=subject]').removeClass("textover");
	}
	showCreditPrompt();
}

function fastpmreplyappendreply() {
	if($('fastpostrefresh') != null) {
		setcookie('fastpostrefresh', $('fastpostrefresh').checked ? 1 : 0, 2592000);
		if($('fastpostrefresh').checked) {
			location.href = 'forum.php?mod=redirect&tid='+tid+'&goto=lastpost&random=' + Math.random() + '#lastpost';
	...
	div.className = '';
	jq("#tlistreply").prepend(div);
	$('fastpmpostsubmit').disabled = false;
	if($('fastthreadmessage')) {
		$('fastthreadmessage').value = '';
	} else {
		editdoc.body.innerHTML = BROWSER.firefox ? '<br />' : '';
	...
	if($('checklen')) {
		$('checklen').innerHTML = 200;
	}
	showCreditPrompt();
}

function succeedhandle_pmpostreply(locationhref, message, param) {
	var pid = param['pid'];
	var tid = param['tid'];
	var from = param['from'];
	var pmtype = param['pmtype'];
	var reppid = param['reppid'] ? param['reppid'] : tid;
	if(pid) {
		if(pmtype == 'index' || pmtype == 'live') {
			ajaxget('forum.php?mod=viewthread&tid=' + tid + '&viewpid=' + pid + '&from=' + from + '&modtype=modreply', 'list_replyid_'+reppid, 'ajaxwaitid', '', null, 'fastpmpostappendreply(\''+pmtype+'\','+reppid+')');
		} else {
			ajaxget('forum.php?mod=viewthread&tid=' + tid + '&viewpid=' + pid + '&from=' + from, 'list_replyid_'+reppid, 'ajaxwaitid', '', null, 'fastpmpostappendreply(\''+pmtype+'\','+reppid+')');
		}
		if(replyreload) {
...
function succeedhandle_fastpmthread(locationhref, message, param) {
	var pid = param['pid'];
	var tid = param['tid'];
	var from = param['from'];
	var modtype = param['modtype'];
	if(pid) {
		ajaxget('forum.php?mod=viewthread&tid=' + tid + '&viewpid=' + pid + '&from=' + from + '&modtype=' + modtype, 'thread_new', 'ajaxwaitid', '', null, 'fastthreadappendreply()');
		if(replyreload) {
			var reloadpids = replyreload.split(',');
			for(i = 1;i < reloadpids.length;i++) {
				ajaxget('forum.php?mod=viewthread&tid=' + tid + '&viewpid=' + reloadpids[i] + '&from=' + from, 'post_' + reloadpids[i], 'ajaxwaitid');
		...
		$('fastpmthreadreturn').className = '';
	} else {
		if(!message) {
			message = '本版回帖需要审核，您的帖子将在通过审核后显示';
		}
		$('thread_new').style.display = $('message').value = $('fastpmthreadreturn').className = '';
		$('fastpmthreadreturn').innerHTML = message;
	}
	if(param['sechash']) {
		updatesecqaa(param['sechash']);
		updateseccode(param['sechash']);
...
function succeedhandle_fastpmpost(locationhref, message, param) {
	var pid = param['pid'];
	var tid = param['tid'];
	var from = param['from'];
	var modtype = param['modtype'];
	if(pid) {
		ajaxget('forum.php?mod=viewthread&tid=' + tid + '&viewpid=' + pid + '&from=' + from + '&modtype=' + modtype, 'thread_new', 'ajaxwaitid', '', null, 'fastpmreplyappendreply()');
		if(replyreload) {
			var reloadpids = replyreload.split(',');
			for(i = 1;i < reloadpids.length;i++) {
				ajaxget('forum.php?mod=viewthread&tid=' + tid + '&viewpid=' + reloadpids[i] + '&from=' + from, 'post_' + reloadpids[i], 'ajaxwaitid');
		...
		$('fastpmpostreturn').className = '';
	} else {
		if(!message) {
			message = '本版回帖需要审核，您的帖子将在通过审核后显示';
		}
		$('thread_new').style.display = $('message').value = $('fastpmthreadreturn').className = '';
		$('fastpmpostreturn').innerHTML = message;
	}
	if(param['sechash']) {
		updatesecqaa(param['sechash']);
		updateseccode(param['sechash']);
...
function appendreply() {
	newpos = fetchOffset($('post_new'));
	document.documentElement.scrollTop = document.body.scrollTop = newpos['top'];
	$('post_new').style.display = '';
	$('post_new').id = '';
	div = document.createElement('div');
	div.id = 'post_new';
