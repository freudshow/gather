...
var postpm = 0;
function fastpmthreadvalidate(theform, noajaxpost) {
	if(postpm) {
		return false;
	}
	postpm = 1;
	setTimeout(function() {postpm = 0}, 2000);
	noajaxpost = !noajaxpost ? 0 : noajaxpost;
	s = '';
	if(typeof fastpostvalidateextra == 'function') {
		var v = fastpostvalidateextra();
	...
	if(bbs_uid <= 0) {
		window.location.href='member.html?mod=logging&action=login';
		return false;
	}
	
	if(theform.fid.value == 0) {
		s = '抱歉，您尚未选择版块';
	} else if(theform.subject.value == '' || theform.subject.value == '请在此输入') {
		s = '抱歉，您尚未输入标题';
		jq('input[name=subject]').attr("value", "");
		theform.subject.focus();
	...
	$('fastpmthreadsubmit').disabled = true;
	theform.message.value = theform.message.value.replace(/([^>=\]"'\/]|^)((((https?|ftp):\/\/)|www\.)([\w\-]+\.)*[\w\-\u4e00-\u9fa5]+\.([\.a-zA-Z0-9]+|\u4E2D\u56FD|\u7F51\u7EDC|\u516C\u53F8)((\?|\/|:)+[\w\.\/=\?%\-&~`@':+!]*)+\.(jpg|gif|png|bmp))/ig, '$1[img]$2[/img]');
	theform.message.value = parseurl(theform.message.value);
	if(!noajaxpost) {
		ajaxpost('fastpmthreadform', 'fastpmthreadreturn', 'fastpmthreadreturn', 'onerror', $('fastpmthreadsubmit'));
		return false;
	} else {
		return true;
	}
...
function fastpmpostvalidate(theform, noajaxpost) {
	noajaxpost = !noajaxpost ? 0 : noajaxpost;
	s = '';
	if(typeof fastpostvalidateextra == 'function') {
		var v = fastpostvalidateextra();
	...
	$('fastpmpostsubmit').disabled = true;
	theform.message.value = theform.message.value.replace(/([^>=\]"'\/]|^)((((https?|ftp):\/\/)|www\.)([\w\-]+\.)*[\w\-\u4e00-\u9fa5]+\.([\.a-zA-Z0-9]+|\u4E2D\u56FD|\u7F51\u7EDC|\u516C\u53F8)((\?|\/|:)+[\w\.\/=\?%\-&~`@':+!]*)+\.(jpg|gif|png|bmp))/ig, '$1[img]$2[/img]');
	theform.message.value = parseurl(theform.message.value);
	if(!noajaxpost) {
		ajaxpost('fastpmpostform', 'fastpmpostreturn', 'fastpmpostreturn', 'onerror', $('fastpmpostsubmit'));
		return false;
	} else {
		return true;
	}
...
var checkManagercount = 0, checkManagertimeout = 2000, checkManagernew_handle;
function checkManagernew(tid, lasttime) {
	var timeout = checkManagertimeout;
	var x = new Ajax();
	x.get('forum.php?mod=ajax&action=managerchecknew&tid=' + tid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			if(!isUndefined(checkManagernew_handle)) {
				clearTimeout(checkManagernew_handle);
			}
			
			jq("#forumnewshow").html("<a href=\"javascript:void(0);\" onclick=\"ajaxget(\'forum.php?mod=ajax&action=managerchecknew&tid="+tid+"&time="+lasttime+"&uncheck=1&inajax=yes\', \'tlistreply\');\">有新的互动消息，点击查看</a>");
			/*
			var table = $('separatorline').parentNode;
			if(!isUndefined(checkManagernew_handle)) {
				clearTimeout(checkManagernew_handle);
			...
			removetbodyrow(table, 'forumnewshow');
			var colspan = table.getElementsByTagName('tbody')[0].rows[0].children.length;
			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=checkManagernew&tid=' + tid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">有新回复的主题，点击查看', 'colspan': colspan }}};
			addtbodyrow(table, ['tbody'], ['forumnewshow'], 'separatorline', checknew);
			*/
		} else {
			if(checkManagercount < 50) {
				if(checkManagercount > 0) {
					var multiple =  Math.ceil(50 / checkManagercount);
					if(multiple < 5) {
						timeout = checkManagertimeout * (5 - multiple + 1);
					}
				}
				jq("#forumnewshow").children().hide();
				checkManagernew_handle = setTimeout(function () {checkManagernew(tid, lasttime);}, timeout);
			}
		}
		checkManagercount++;
	});
