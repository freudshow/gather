...
static char gRecBuf_xmz[MAX_REC_MC52i_BYTENUM+100];//ÕâÀï+100ÊÇÈßÓà
...
/*
******************************************************************************
* º¯ÊýÃû³Æ£ºvoid GPRS_FileSend(uint8 Dev, FILE *fp)
* Ëµ	Ã÷£º Í¨¹ýGPRS¶Ë¿Ú£¬·¢ËÍÒ»¸öÎÄ¼þÄÚÈÝ¡£
* ²Î	Êý£º 
******************************************************************************
*/
void GPRS_FileSend(uint8 Dev, FILE *fp)
{
	...
	//¼ÆËãÎÄ¼þ´óÐ¡¡£
	fseek(fp,0,2);			//Î»ÖÃÖ¸Õëµ½ÎÄ¼þÄ©Î²
	l32FileSize = ftell(fp);	//¼ÆËãÎÄ¼þ´óÐ¡
	fseek(fp,0,0);			//Î»ÖÃÖ¸Õëµ½ÎÄ¼þ¿ªÊ¼
	lu32SendNum = SEND_TEMPBUF_SIZE;

	for(i=0;i<(l32FileSize/SEND_TEMPBUF_SIZE + 1);i++){
		if(i == l32FileSize/SEND_TEMPBUF_SIZE){ //×îºóÒ»°ü
...
/********************************************************************************************************
**  º¯ Êý  Ãû ³Æ: Fun_GprsIpd_xmz       									                           **
**  º¯ Êý  ¹¦ ÄÜ: Î÷ÃÅ×ÓÄ£¿é½ÓÊÕÍøÂç´«ÊäÊý¾Ý´¦Àíº¯Êý¡£							                                           **			
**  Êä Èë  ²Î Êý: pdata												                                   **
**  Êä ³ö  ²Î Êý: none											                                       **
**  ·µ   »Ø   Öµ: none																			   	   **
**  ±¸	    ×¢: 						                                                               **
...
{
	//±¾ÈÎÎñÃ»ÓÐÈÎÎñ¹·,Ô­ÒòÖ®Ò»ÊÇËü¿ÉÄÜÔÚÒ»¶ÎÊ±¼ä´¦ÔÚ¹ÒÆð×´Ì¬
	uint8 err;
	uint16 len;
	uint8 Count = 0;
	GPRS_RUN_STA GprsRunSta;
		...
		if(err==0){
			...
			do{
				err = GprsGetIPDATA_jh(gRecBuf_xmz,0,&len);
	  			if(err){
					//ÖØÊÔÒ»´Î 
					//¿ÉÄÜ¶Ôµ÷ÊÔ¹ý³ÌÖÐ¶Ïµã(»ò±¾µØ¿ÚÀ´µÄÊý¾Ý)µ¼ÖÂ´íÎó¶ø³¹µ×ÖÐ¶ÏÎ÷ÃÅ×ÓÄ£¿é·¢µÄ^SISR: 1, 1(Êý¾Ý»¹Ã»ÓÐÍêÈ«¶Á¿ÕÊ±)
					debug_err(gDebugModule[GPRS_MODULE],"%s %d err=%d\n",__FUNCTION__,__LINE__,err);	
					OSTimeDly(OS_TICKS_PER_SEC/8);
	  				err = GprsGetIPDATA_jh(gRecBuf_xmz,0,&len);
					if(err){
				...
				if(len > 400){gUpdateBegin = 1;}/*½øÈëÖ÷¶¯¶ÁÈ¡Êý¾ÝÄ£Ê½*/
				if(len == 0)
					{Count++;}
				else
					{Count = 0;}
				if(Count > 10){gUpdateBegin = 0;}/*Á¬Ðø10´ÎÎÞÊý¾Ý½øÈë²éÑ¯½ÓÊÜÊý¾ÝÄ£Ê½*/
...
/*
  ******************************************************************************
  * º¯ÊýÃû³Æ£º int pthread_GPRS_IPD(void)
  * Ëµ    Ã÷£º »ñÈ¡GPRSÍøÂç´«ÊäÊý¾ÝµÄÏß³Ì¡£
  * ²Î    Êý£º ÎÞ
  ******************************************************************************
*/
...
/*
******************************************************************************
* º¯ÊýÃû³Æ£º void pthread_GprsDataDeal(void)
* Ëµ	Ã÷£º Í¨¹ýGPRS´«Êäµ½¼¯ÖÐÆ÷µÄÊý¾Ý´¦ÀíÏß³Ì¡£
* ²Î	Êý£º 
******************************************************************************
*/
